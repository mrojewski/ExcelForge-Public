...
guardian_outline_glyphs = [a for a in guardian_glyphs if 'guardian-outline-glyph' in a.get('class', '')]
guardian_color_glyphs = [a for a in guardian_glyphs if 'guardian-outline-glyph' not in a.get('class', '')]
assert len(guardian_color_glyphs) == 6 and all(a.get('fill') == '#FFFFFF' for a in guardian_color_glyphs), 'Color Guardian glyphs must directly declare white fill'
assert len(guardian_outline_glyphs) == 6 and all(a.get('fill') == '#0C6B3B' for a in guardian_outline_glyphs), 'Outline Guardian glyphs must directly declare EF dark green fill'
...
assert 'pilot-guardian-mark' in html and 'guardian-outline-tile' in html and 'guardian-mono-tile' not in html, 'Missing secondary outline Guardian mark in Pilot'
assert 'padding-left:14px' in css, 'Pilot heading breathing room missing'
assert html.count('class="platform-identifier"') == 2 and '>W</span>' in html and '>M</span>' in html, 'Neutral platform identifiers missing'
assert 'border:1.5px solid var(--block-4)' in css and 'border-radius:7px' in css, 'Platform identifiers must use the shared outline system'
assert 'transform:translateY(-14px)' in css, 'Desktop EF hero optical alignment correction missing'
assert css.count('width:10px') >= 2 and css.count('height:10px') >= 2, 'Guardian benefit accents must be 10 px on desktop and mobile'
